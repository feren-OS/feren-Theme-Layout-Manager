...
class Module:
    ...
    def on_module_selected(self):
	    ...
	    try:
		settings = page.add_section(_("Load/Save Theme/Layout Config"))
		widget = SettingsWidget()
	        widget.set_spacing(40)
		
	        savelytbtn = Gtk.Button("Save the Current Layout & Theme Config")
		savelytbtn.connect("clicked", self.on_savelytbtn)
		loadlytbtn = Gtk.Button("Load a Saved Layout & Theme Config")
		loadlytbtn.connect("clicked", self.on_loadlytbtn)
	        widget.pack_start(savelytbtn, True, False, 0)
	        widget.pack_end(loadlytbtn, True, False, 0)
	        settings.add_row(widget)
		widget = SettingsWidget()
	        widget.set_spacing(5)
		disclaimertxt=Gtk.Label()
		disclaimertxt.set_markup("<i>The Window May Appear Behind this window</i>")
	        widget.pack_start(disclaimertxt, True, False, 0)
	        widget.pack_end(disclaimertxt, True, False, 0)
	        settings.add_row(widget)
	    ...
	    try:
                ...
                widget = SettingsWidget()
                content = self.sidePage.content_box.c_manager.get_c_widget("themes")
    ...
    def on_loadlytbtn(self, widget):								 
        try:
            os.system("feren-theme-loader")
        except:
            mde = Gtk.MessageDialog(self, 0, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, "An error has occured")
            mde.run()
            mde.destroy()

    def on_savelytbtn(self, widget):								 
        try:
            os.system("feren-theme-saver")
        except:
            mde = Gtk.MessageDialog(self, 0, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, "An error has occured")
            mde.run()
            mde.destroy()
